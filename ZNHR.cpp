...
void Element::Element_ZNHR()
{
	Identifier = "DEFAULT_PT_ZNHR";
	Name = "ZNHR";
...
static int update(UPDATE_FUNC_ARGS)
{
		...
		for (ry=-2; ry<3; ry++)
			if (BOUNDS_CHECK && (rx || ry))
			{
				...
				if (rt != PT_ZNHR && rt != PT_CAUS)
				{
					...
					{
						if (sim->parts_avg(i, ID(r),PT_GLAS)!= PT_GLAS)//GLAS protects stuff from ZNHR
						{
	...
	for (trade = 0; trade<2; trade++)
	{
		...
		if (BOUNDS_CHECK && (rx || ry))
		{
			...
			if (!r)
				continue;
			if (TYP(r) == PT_ZNHR && (parts[i].life > parts[ID(r)].life) && parts[i].life>0)//diffusion
			{
